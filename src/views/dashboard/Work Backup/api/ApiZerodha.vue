<template>
  ...
  >
    ...
    <b-img
      fluid
      class="mb-2"
      :src="require('@/assets/images/brokers/alt/zerodha.png')"
    />
    <b-button
      id="addkiteapi"
      v-ripple.400="'rgba(255, 255, 255, 0.15)'"
      v-b-modal.modal-addkiteapi
      block
    ...
    <b-modal
      ...
      centered
      size="lg"
      title="Add Kite API Credentials"
      no-stacking
      @show="resetModal"
      @hidden="resetModal"
      @ok="handleOk"
    ...
    <b-modal
      id="modal-addkiteupt"
      ref="zerodhaupt-modal"
      cancel-variant="outline-secondary"
      ok-title="Submit"
      cancel-title="Close"
      centered
      size="lg"
      title="Add Kite Web Credentials"
      @show="resetModal"
      @hidden="resetModal"
      @ok="handleOk1"
    >
...
export default {
  ...
  methods: {
    validationForm() {
      this.$refs.simpleRules.validate().then(success => {
        if (success) {
          ...
          this.$nextTick(() => {
            this.$refs['zerodha-modal'].toggle('#addkiteapi')
          })
    ...
    validationForm1() {
      this.$refs.simpleRules.validate().then(success => {
        if (success) {
          ...
          this.$nextTick(() => {
            this.$refs['zerodhaupt-modal'].toggle('#addkiteupt')
          })
          // eslint-disable-next-line
          // alert('API Details Submitted.')
    ...
    handleSubmit() {
      ...
      // this.apidata.push(this.appname)
      // this.apidata.push(this.apikey)
      // this.apidata.push(this.apisecret)
      // console.log(this.appname, this.apikey, this.apisecret)
