  ...
  >
    ...
    >
      <feather-icon
        ...
        class="mr-25"
      />
      {{ SAMapicount }} Saved API
    </b-badge>
    <b-img
      fluid
      class="mb-2"
      :src="require('@/assets/images/brokers/samco1.png')"
    />
    <b-button
      id="addsamcoapi"
      v-ripple.400="'rgba(255, 255, 255, 0.15)'"
      v-b-modal.modal-addsamcoapi
      block
    ...
    <b-modal
      ...
      centered
      size="lg"
      title="Add Samco API Credentials"
      no-stacking
      @show="resetModal"
      @hidden="resetModal"
      @ok="handleOk"
    ...
    <b-modal
      ...
      centered
      size="lg"
      title="Add Samco Web Credentials"
      @show="resetModal"
      @hidden="resetModal"
      @ok="handleOk1"
    >
    ...
    validationForm() {
      this.$refs.simpleRules.validate().then(success => {
        if (success) {
          ...
          this.$nextTick(() => {
            this.$refs['samco-modal'].toggle('#addsamcoapi')
          })
    ...
    validationForm1() {
      this.$refs.simpleRules.validate().then(success => {
        if (success) {
          ...
          this.$nextTick(() => {
            this.$refs['samcoupt-modal'].toggle('#addsamcoupt')
          })
          // eslint-disable-next-line
          // alert('API Details Submitted.')
