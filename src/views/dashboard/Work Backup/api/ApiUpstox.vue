<template>
  ...
  >
    ...
    <b-img
      fluid
      class="mb-2"
      :src="require('@/assets/images/brokers/alt/upstox.png')"
    />
    <b-button
      id="addupstoxapi"
      v-ripple.400="'rgba(255, 255, 255, 0.15)'"
      v-b-modal.modal-addupstoxapi
      block
    ...
    <b-modal
      ...
      centered
      size="lg"
      title="Add Upstox API Credentials"
      no-stacking
      @show="resetModal"
      @hidden="resetModal"
      @ok="handleOk"
    ...
    <b-modal
      ...
      centered
      size="lg"
      title="Add Upstox Web Credentials"
      @show="resetModal"
      @hidden="resetModal"
      @ok="handleOk1"
    >
    ...
    validationForm() {
      this.$refs.simpleRules.validate().then(success => {
        if (success) {
          ...
          this.$nextTick(() => {
            this.$refs['upstox-modal'].toggle('#addupstoxapi')
          })
    ...
    validationForm1() {
      this.$refs.simpleRules.validate().then(success => {
        if (success) {
          ...
          this.$nextTick(() => {
            this.$refs['upstoxupt-modal'].toggle('#addupstoxupt')
          })
          // eslint-disable-next-line
          // alert('API Details Submitted.')
