  ...
  >
    ...
    >
      <feather-icon
        ...
        class="mr-25"
      />
      {{ UXapicount }} Saved API
    </b-badge>
    <b-img
      fluid
      class="mb-2"
      :src="require('@/assets/images/brokers/angelbroking.png')"
    />
    <b-button
      id="addangelbrokingapi"
      v-ripple.400="'rgba(255, 255, 255, 0.15)'"
      v-b-modal.modal-addangelbrokingapi
      block
    ...
    <!-- modal addangelbrokingapi-->
    <b-modal
      id="modal-addangelbrokingapi"
      ref="angelbroking-modal"
      cancel-variant="outline-secondary"
      ok-title="Submit"
      cancel-title="Close"
      centered
      size="lg"
      title="Add AngelBroking API Credentials"
      no-stacking
      @show="resetModal"
      @hidden="resetModal"
      @ok="handleOk"
    >
      <b-row>
        <b-col md="12">
          ...
          <b-button
            id="addangelbrokingupt"
            v-ripple.400="'rgba(255, 255, 255, 0.15)'"
            v-b-modal.modal-addangelbrokingupt
            block
            variant="gradient-primary"
          >
            <feather-icon
    ...
    <!-- modal addangelbrokingupt-->
    <b-modal
      id="modal-addangelbrokingupt"
      ref="angelbrokingupt-modal"
      cancel-variant="outline-secondary"
      ok-title="Submit"
      cancel-title="Close"
      centered
      size="lg"
      title="Add AngelBroking Web Credentials"
      @show="resetModal"
      @hidden="resetModal"
      @ok="handleOk1"
    >
    ...
    validationForm() {
      this.$refs.simpleRules.validate().then(success => {
        if (success) {
          ...
          this.$nextTick(() => {
            this.$refs['angelbroking-modal'].toggle('#addangelbrokingapi')
          })
    ...
    validationForm1() {
      this.$refs.simpleRules.validate().then(success => {
        if (success) {
          ...
          this.$nextTick(() => {
            this.$refs['angelbrokingupt-modal'].toggle('#addangelbrokingupt')
          })
          // eslint-disable-next-line
          // alert('API Details Submitted.')
