<template>
  ...
  >
    ...
    <b-img
      ...
      overlay
      class="mb-2"
      :src="require('@/assets/images/brokers/alt/aliceblue.png')"
    />
    <b-button
      id="addaliceblueapi"
      v-ripple.400="'rgba(255, 255, 255, 0.15)'"
      v-b-modal.modal-addaliceblueapi
      block
    ...
    <!-- modal addaliceblueapi-->
    <b-modal
      id="modal-addaliceblueapi"
      ref="aliceblue-modal"
      ...
      centered
      size="lg"
      title="Add AliceBlue API Credentials"
      no-stacking
      @show="resetModal"
      @hidden="resetModal"
      @ok="handleOk"
      ...
      <b-row>
        <b-col md="12">
          ...
          <b-button
            id="addaliceblueupt"
            v-ripple.400="'rgba(255, 255, 255, 0.15)'"
            v-b-modal.modal-addaliceblueupt
            block
            variant="gradient-primary"
          >
            <feather-icon
    ...
    <!-- modal addaliceblueupt-->
    <b-modal
      id="modal-addaliceblueupt"
      ref="aliceblueupt-modal"
      cancel-variant="outline-secondary"
      ok-title="Submit"
      cancel-title="Close"
      centered
      size="lg"
      title="Add AliceBlue Web Credentials"
      @show="resetModal"
      @hidden="resetModal"
      @ok="handleOk1"
    >
    ...
    validationForm() {
      this.$refs.simpleRules.validate().then(success => {
        if (success) {
          ...
          this.$nextTick(() => {
            this.$refs['aliceblue-modal'].toggle('#addaliceblueapi')
          })
    ...
    validationForm1() {
      this.$refs.simpleRules.validate().then(success => {
        if (success) {
          ...
          this.$nextTick(() => {
            this.$refs['aliceblueupt-modal'].toggle('#addaliceblueupt')
          })
          // eslint-disable-next-line
          // alert('API Details Submitted.')
