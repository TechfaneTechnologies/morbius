  ...
  >
    ...
    >
      <feather-icon
        ...
        class="mr-25"
      />
      {{ BNapicount }} Saved API
    </b-badge>
    <b-img
      fluid
      class="mb-2"
      :src="require('@/assets/images/brokers/binance1.png')"
    />
    <b-button
      id="addbinanceapi"
      v-ripple.400="'rgba(255, 255, 255, 0.15)'"
      v-b-modal.modal-addbinanceapi
      block
    ...
    <b-modal
      ...
      centered
      size="lg"
      title="Add Binance API Credentials"
      no-stacking
      @show="resetModal"
      @hidden="resetModal"
      @ok="handleOk"
    ...
    <!-- modal addbinanceupt-->
    <b-modal
      id="modal-addbinanceupt"
      ref="binanceupt-modal"
      cancel-variant="outline-secondary"
      ok-title="Submit"
      cancel-title="Close"
      centered
      size="lg"
      title="Add Binance Web Credentials"
      @show="resetModal"
      @hidden="resetModal"
      @ok="handleOk1"
    >
    ...
    validationForm() {
      this.$refs.simpleRules.validate().then(success => {
        if (success) {
          ...
          this.$nextTick(() => {
            this.$refs['binance-modal'].toggle('#addbinanceapi')
          })
    ...
    validationForm1() {
      this.$refs.simpleRules.validate().then(success => {
        if (success) {
          ...
          this.$nextTick(() => {
            this.$refs['binanceupt-modal'].toggle('#addbinanceupt')
          })
          // eslint-disable-next-line
          // alert('API Details Submitted.')
