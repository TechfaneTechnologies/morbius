<template>
  ...
  >
    ...
    <b-img
      fluid
      class="mb-2"
      :src="require('@/assets/images/brokers/alt/fyers.png')"
    />
    <b-button
      id="addfyersapi"
      v-ripple.400="'rgba(255, 255, 255, 0.15)'"
      v-b-modal.modal-addfyersapi
      block
    ...
    <b-modal
      ...
      centered
      size="lg"
      title="Add Fyers API Credentials"
      no-stacking
      @show="resetModal"
      @hidden="resetModal"
      @ok="handleOk"
    ...
    <b-modal
      ...
      centered
      size="lg"
      title="Add Fyers Web Credentials"
      @show="resetModal"
      @hidden="resetModal"
      @ok="handleOk1"
    >
    ...
    validationForm() {
      this.$refs.simpleRules.validate().then(success => {
        if (success) {
          ...
          this.$nextTick(() => {
            this.$refs['fyers-modal'].toggle('#addfyersapi')
          })
    ...
    validationForm1() {
      this.$refs.simpleRules.validate().then(success => {
        if (success) {
          ...
          this.$nextTick(() => {
            this.$refs['fyersupt-modal'].toggle('#addfyersupt')
          })
          // eslint-disable-next-line
          // alert('API Details Submitted.')
