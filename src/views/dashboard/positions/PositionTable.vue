    ...
    >
      ...
      >
        <b-button
          ...
          @click="exitAll()"
        >
          Exit {{ rowSelection.length != rows.length ? rowSelection.length+' Positions':'All' }}
        </b-button>
        <b-button
          v-ripple.400="'rgba(186, 191, 199, 0.15)'"
          v-b-modal.modal-convert
          variant="gradient-primary"
          size="sm"
          pill
        >
          Convert {{ rowSelection.length != rows.length ? rowSelection.length+' Positions':'All' }}
        </b-button>
        <!-- modal login-->
        <b-modal
          id="modal-convert"
      ...
      >
        ...
        <span
          ...
          :class="((props.row.net_quantity) != 0) ? 'font-weight-bolder':'text-muted'"
        >
        ...
        <span
          ...
          :class="((props.row.net_quantity) != 0) ? mtmVariant(props.row.m2m)+' '+'font-weight-bolder':'text-muted'"
        >
...
export default {
  ...
  data() {
    return {
      ...
      convertval: '',
      convertoption: [{ convert: 'NRML2MIS' }, { convert: 'MIS2NRML' }],
    }
  },
  computed: {
    pnlVariant() {
  ...
  methods: {
    ...
    exitAll() {
      this.$swal({
        ...
        icon: 'warning',
        html:
          `This will exit ${this.rowSelection.length !== this.rows.length ? `${this.rowSelection.length} Positions.` : 'All Positions.'}`,
    ...
    convertAll() {
      if (this.convertval) {
        this.$swal({
          ...
          icon: 'warning',
          html:
            `This will convert ${this.rowSelection.length !== this.rows.length ? `${this.rowSelection.length} Positions.` : 'All Positions.'} from ${this.convertval.convert === 'NRML2MIS' ? 'NMRL to MIS' : 'MIS to NRML'}`,
          showCloseButton: true,
          showCancelButton: true,
          focusConfirm: false,
