<template>
  ...
  >
    <b-img
      fluid
      class="mb-2"
      :src="require('@/assets/images/brokers/iifllogo.png')"
    />
    <b-badge
      href="#"
      variant="primary"
    ...
    <b-modal
      ...
      centered
      size="lg"
      title="Add IIFL API Credentials"
      no-stacking
      @show="resetModal"
      @hidden="resetModal"
      @ok="handleOk"
    ...
    <b-modal
      ...
      centered
      size="lg"
      title="Add IIFL Web Credentials"
      @show="resetModal"
      @hidden="resetModal"
      @ok="handleOk1"
    >
    ...
    validationForm() {
      this.$refs.simpleRules.validate().then(success => {
        if (success) {
          ...
          this.$nextTick(() => {
            this.$refs['iifl-modal'].toggle('#addiiflapi')
          })
    ...
    validationForm1() {
      this.$refs.simpleRules.validate().then(success => {
        if (success) {
          ...
          this.$nextTick(() => {
            this.$refs['iiflupt-modal'].toggle('#addiiflupt')
          })
          // eslint-disable-next-line
          // alert('API Details Submitted.')
